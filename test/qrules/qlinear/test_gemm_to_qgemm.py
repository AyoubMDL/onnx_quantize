...
def _test_gemm_to_qgemm(rng, model, qconfig):
    ...
    model = onnxscript.rewriter.rewrite(model, gemm_to_qlinear_gemm_rules)
    ...
    for org_node, node in zip(clone.graph, model.graph, strict=True):
        ...
        if len(org_node.inputs) == 3:
            expected_op_type = get_qfunction("Gemm", qconfig).__name__
        else:
            expected_op_type = get_qfunction("MatMul", qconfig).__name__
    ...
    for expected_op in {get_qfunction("MatMul", qconfig), get_qfunction("Gemm", qconfig)}:
...
def test_gemm_to_qgemm_weights_inputs_outputs(rng, strategy):
