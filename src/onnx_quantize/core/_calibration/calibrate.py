...
from onnx_quantize.core._qconfig import QActivationArgs, QConfig
...
def calibrate_model(ir_model: ir.Model, qconfig: QConfig):
    ...
    preprocessing_needs_inputs = any(pre.requires_calibration for pre in qconfig.preprocessors)
    algorithm_needs_inputs = (
        qconfig.weights is not None and qconfig.weights.algorithm.requires_calibration
    )

    values_to_calibrate = _get_values_to_calibrate(
        nodes_to_calibrate,
        augment_inputs=calibrate_inputs or algorithm_needs_inputs or preprocessing_needs_inputs,
    ...
    if algorithm_needs_inputs or preprocessing_needs_inputs:
